...
> 💡 **Script paths are relative to `.aim-agents/` directory**

#### 1. Run All Verifications (RECOMMENDED)

```bash
# SINGLE COMMAND - Runs all checks in priority order:
python .aim-agents/scripts/verify_all.py . --url http://localhost:3000
...
# P0: Security Scan
python .aim-agents/skills/vulnerability-scanner/scripts/security_scan.py .

# P1: UX Audit
python .aim-agents/skills/frontend-design/scripts/ux_audit.py .

# P3: Lighthouse (requires running server)
python .aim-agents/skills/performance-profiling/scripts/lighthouse_audit.py http://localhost:3000

# P4: Playwright E2E (requires running server)
python .aim-agents/skills/webapp-testing/scripts/playwright_runner.py http://localhost:3000 --screenshot
```
