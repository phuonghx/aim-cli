...
1. **Load memory-system skill** — Read `.aim-agents/skills/memory-system/SKILL.md` first
...
- Memory location: .aim-agents/memory/

WORKFLOW:
1. CLASSIFY the information type: user | feedback | project | reference
2. CHECK if relevant topic file exists in .aim-agents/memory/
3. SAVE to appropriate topic file (create if needed)
4. UPDATE .aim-agents/memory/MEMORY.md index with one-line pointer
...
File: .aim-agents/memory/[topic-file].md
