...
Usage:
    python .aim-agents/scripts/auto_preview.py start [port]
    python .aim-agents/scripts/auto_preview.py stop
    python .aim-agents/scripts/auto_preview.py status
"""
...
AGENT_DIR = Path(".aim-agents") if Path(".aim-agents").exists() else Path(".aim-agents")
...
def start_server(port=3000):
    ...
    # Resolve the executable on PATH (e.g. npm -> npm.cmd on Windows). Launching
    # with the full path means we never need shell=True, so there is no
    # shell-injection surface.
    resolved = shutil.which(cmd[0])
    if not resolved:
        print(f"❌ '{cmd[0]}' was not found on your PATH.")
        sys.exit(1)
    cmd = [resolved] + cmd[1:]
