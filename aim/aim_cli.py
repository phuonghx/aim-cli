...
def _is_within(base, target):
    """True when `target` resolves to a path inside `base` (inclusive).
    Blocks `../` traversal escapes. Returns False across drives (Windows)."""
...
_ALL_AGENTS = [
    ...
    ("antigravity", "Antigravity",     "ANTIGRAVITY.md, ~/.gemini/plugins/"),
...
_GIT_STRATEGIES = [
    ("track-all",  "Track all — commit generated files to git (recommended for teams)"),
    ("ignore-all", "Ignore all — add to .gitignore (each dev runs `aim sync`)"),
    ("rules-only", "Track rules only — commit CLAUDE.md etc, ignore slash commands"),
]

# ---------------------------------------------------------------------------
# Interactive terminal picker (zero-dependency, stdlib only)
# ---------------------------------------------------------------------------

def _read_key():
    """Read a single keypress. Returns special strings for arrow keys."""
...
def _interactive_checkbox(title, options, defaults=None):
    """Interactive multi-select checkbox picker.

    Controls: ↑↓ navigate, Space toggle, A select all, N deselect all, Enter confirm
    """
    n = len(options)
    selected = list(defaults) if defaults else [True] * n
    cursor = 0
    out = sys.stdout

    # Strip leading newlines — they go before the redrawn block
    prefix = ""
    clean_title = title
    while clean_title.startswith("\n"):
        prefix += "\n"
        clean_title = clean_title[1:]
    total_lines = 2 + n  # title + hint + options

    def _redraw(is_first=False):
        buf = []
        if not is_first:
            buf.append(f"\033[{total_lines}A")
        else:
            buf.append(prefix)
        buf.append(f"\r\033[2K\033[1m{clean_title}\033[0m\r\n")
        buf.append(f"\r\033[2K\033[2m  (\u2191\u2193 navigate, Space toggle, A all, N none, Enter confirm)\033[0m\r\n")
        for i, (_k, label, desc) in enumerate(options):
            marker = "\033[32m\u2713\033[0m" if selected[i] else " "
            pointer = "\033[36m\u276f\033[0m" if i == cursor else " "
            dim_desc = f"\033[2m{desc}\033[0m" if desc else ""
            buf.append(f"\r\033[2K  {pointer} [{marker}] {label:<20s} {dim_desc}\r\n")
        out.write("".join(buf))
        out.flush()

    out.write("\033[?25l")
    out.flush()
    _redraw(is_first=True)
    ...
    # Final clean render (replace interactive with static result)
    out.write(f"\033[{total_lines}A\r\033[J")
    result_labels = []
    for i, (_k, label, _d) in enumerate(options):
        marker = "\033[32m\u2713\033[0m" if selected[i] else "\033[31m\u2717\033[0m"
        out.write(f"  [{marker}] {label}\r\n")
        if selected[i]:
            result_labels.append(label)
    out.write(f"\033[1m[+] Selected: {', '.join(result_labels) or 'none'}\033[0m\r\n")
    out.flush()

    return {options[i][0]: selected[i] for i in range(n)}


def _interactive_radio(title, options, default=0):
    """Interactive single-select radio picker.
    ...
    def _redraw(is_first=False):
        ...
        if not is_first:
            ...
            buf.append(prefix)
        buf.append(f"\r\033[2K\033[1m{clean_title}\033[0m\r\n")
        buf.append(f"\r\033[2K\033[2m  (\u2191\u2193 navigate, Enter select)\033[0m\r\n")
        for i, (_k, desc) in enumerate(options):
            marker = "\033[32m\u25cf\033[0m" if i == cursor else "\u25cb"
            pointer = "\033[36m\u276f\033[0m" if i == cursor else " "
            buf.append(f"\r\033[2K  {pointer} {marker} {desc}\r\n")
        out.write("".join(buf))
        out.flush()
    ...
    # Final
    out.write(f"\033[{total_lines}A\r\033[J")
    _, desc = options[cursor]
    out.write(f"  \033[32m\u25cf\033[0m {desc}\r\n")
    out.flush()

    return options[cursor][0]


def _can_interactive():
...
# ---------------------------------------------------------------------------
# Prompt functions (use interactive when available, fall back to numbered)
# ---------------------------------------------------------------------------

def _prompt_agents(preselected=None):
    """Agent selection. Interactive checkbox when possible, numbered fallback."""
    if preselected is not None:
        selected = set(preselected)
        return {key: (key in selected) for key, _, _ in _ALL_AGENTS}

    if _can_interactive():
        try:
            return _interactive_checkbox(
                "\nSelect target AI agents:",
                _ALL_AGENTS,
                defaults=[True] * len(_ALL_AGENTS),
            )
        except (Exception, KeyboardInterrupt):
            print()  # fallthrough to numbered

    # Fallback: numbered input
    print("\nSelect target AI agents (comma-separated numbers, Enter for all):")
    for i, (key, label, files) in enumerate(_ALL_AGENTS, 1):
        print(f"  [{i}] {label:<20s} ({files})")
    print()

    try:
        raw = input("Agents [1,2,3,4,5,6,7]: ").strip()
    except (EOFError, KeyboardInterrupt):
        raw = ""

    if not raw:
        return {key: True for key, _, _ in _ALL_AGENTS}

    try:
        nums = [int(x.strip()) for x in raw.split(",") if x.strip()]
    except ValueError:
        print("[!] Invalid input, enabling all agents.")
        return {key: True for key, _, _ in _ALL_AGENTS}

    result = {}
    for i, (key, _, _) in enumerate(_ALL_AGENTS, 1):
        result[key] = (i in nums)
    enabled = [l for k, l, _ in _ALL_AGENTS if result.get(k)]
    print(f"[+] Selected: {', '.join(enabled)}")
    return result


def _prompt_git_strategy(preset=None):
    """Git tracking strategy. Interactive radio when possible."""
    if preset is not None:
        return preset

    if _can_interactive():
        try:
            return _interactive_radio(
                "\nGit tracking strategy for generated files:",
                _GIT_STRATEGIES,
                default=0,
            )
        except (Exception, KeyboardInterrupt):
    ...
    # Fallback
    print("\nGit tracking strategy for generated files:")
    for i, (key, desc) in enumerate(_GIT_STRATEGIES, 1):
        print(f"  [{i}] {desc}")
    print()

    try:
        raw = input("Strategy [1]: ").strip()
    except (EOFError, KeyboardInterrupt):
        raw = ""
...
def _prompt_skill_commands(preset=None):
    """Ask whether to generate skill slash commands."""
    if preset is not None:
        return preset

    if _can_interactive():
        try:
            key = _interactive_radio(
                "\nEnable /aim-<skill> slash commands?",
                [("yes", "Yes — generate /aim-* commands for all enabled agents"),
                 ("no",  "No  — skip slash command generation")],
                default=0,
            )
            return key == "yes"
        except (Exception, KeyboardInterrupt):
    ...
    try:
        raw = input("\nEnable /aim-<skill> slash commands? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        raw = ""
    return raw != "n"
...
def cmd_init(args):
    ...
    if getattr(args, "agents", None):
        preselected_agents = [a.strip() for a in args.agents.split(",")]
    elif getattr(args, "all_agents", False):
        preselected_agents = [key for key, _, _ in _ALL_AGENTS]

    git_preset = None
    if getattr(args, "git_ignore", False):
        git_preset = "ignore-all"
    elif getattr(args, "git_track", False):
        git_preset = "track-all"

    cmd_preset = None
    if getattr(args, "no_commands", False):
        cmd_preset = False

    # Check if stdin is interactive (not piped)
    is_interactive = sys.stdin.isatty() and preselected_agents is None

    if is_interactive:
        agents = _prompt_agents()
        git_strategy = _prompt_git_strategy(git_preset)
        skill_commands = _prompt_skill_commands(cmd_preset)
    else:
        agents = _prompt_agents(preselected_agents)
        git_strategy = git_preset or "track-all"
        skill_commands = cmd_preset if cmd_preset is not None else True
    ...
    if not os.path.exists(CONFIG_PATH):
        ...
        if os.path.exists(template_path):
            ...
            config_data["skillCommands"] = skill_commands
            config_data["gitTracking"] = git_strategy
            with open(CONFIG_PATH, "w", encoding="utf-8") as f:
                json.dump(config_data, f, indent=2)
            print("[+] Created config.json with agent selection.")
        else:
            print("[-] Warning: config.json template not found.")
    else:
        # Update existing config with new agent selection
        ...
        with open(CONFIG_PATH, "w", encoding="utf-8") as f:
            json.dump(config_data, f, indent=2)
        print("[*] Updated config.json with new agent selection.")
    ...
    # Copy Claude base slash commands (only if Claude is enabled)
    if agents.get("claude", True):
...
"""
        ...
        if not _is_within(DOCS_DIR, doc_file_path):
            print(f"[-] Refusing to read outside the docs directory: {args.path}")
            sys.exit(1)
    ...
    if args.mem_action == "add":
        try:
            new_mem, _memories = core.add_memory(
                args.content, args.category, args.layer,
        ...
        ctx = core.get_memory_context(query=args.query, limit=args.limit)
        if not ctx:
            print("[*] No relevant memories recorded yet.")
            return
        print(ctx)
...
def cmd_spec(args):
    ...
    elif args.spec_action == "new":
        try:
            result = core.create_spec(args.name, with_task=not args.no_task)
        except ValueError as e:
            print(f"[-] {e}")
            sys.exit(1)
        print(f"[+] Created spec '{result['name']}' under .ai-context/docs/specs/{result['slug']}/")
        for _fname, rel in result["docs"].items():
            print(f"  - @doc/{rel}")
        if result.get("taskId"):
            print(f"  - umbrella task: task-{result['taskId']} (linked via --spec)")
        print("[*] Next: fill requirements (EARS) -> design -> break tasks.md "
              "into tracked tasks (MCP `decompose_prd` or `aim task create`).")
...
"""
        ...
        for action in actions:
            ...
            if a_type == "add":
                ...
                if not _is_within(dest_root, final_dest_path):
                    print(f"[-] Refusing to write outside the destination directory: {rendered_path}")
                    continue
...
# ==========================================
# 12.6. UPGRADE COMMAND
# ==========================================
def _update_check():
    try:
        from aim import update_check
    except ImportError:
        import update_check
    return update_check

def cmd_upgrade(args):
    """Upgrade AIM in place via pip, then refresh the update-check cache."""
    import subprocess
    uc = _update_check()
    repo_url = f"git+https://github.com/{uc.REPO}.git"
    cmd = [sys.executable, "-m", "pip", "install", "--upgrade", repo_url]
    print(f"[*] Upgrading AIM:  {' '.join(cmd)}")
    try:
        rc = subprocess.call(cmd)
    except OSError as e:
        print(f"[-] Could not start pip: {e}")
        sys.exit(1)
    if rc != 0:
        print("[-] Upgrade failed. See the pip output above.")
        sys.exit(rc)
    print("[+] AIM upgraded. Run `aim --version` to confirm.")
    try:
        uc.get_latest_known(__version__, force=True)  # stop further nagging
    except Exception:
        pass

# ==========================================
# 12.7. LINT COMMAND (skill quality gate)
# ==========================================
def cmd_lint(args):
    core = _core()
    root = args.path or "."
    if not os.path.isdir(root):
        ...
        sys.exit(1)
    files = core.find_skill_files(root)
    if not files:
        print(f"[*] No SKILL.md files found under '{root}'.")
        return
    if getattr(args, "fix", False):
        results = core.normalize_skills(root)
        fixed = [r for r in results if r["status"] == "fixed"]
        skipped = [r for r in results if r["status"] == "skipped"]
        for r in fixed:
            print(f"FIXED    {os.path.relpath(r['path'], root)}: {r['detail']}")
        for r in skipped:
            print(f"SKIP     {os.path.relpath(r['path'], root)}: {r['detail']}")
        print(f"[+] Normalized {len(fixed)} skill(s); {len(skipped)} skipped.")
        print("-" * 50)
    findings = core.lint_skills(root)
    if not findings:
        print(f"[+] {len(files)} skill(s) linted - all clean!")
        return
    order = {"error": 0, "warning": 1, "info": 2}
    findings.sort(key=lambda f: (order.get(f["level"], 3), f["path"]))
    counts = {"error": 0, "warning": 0, "info": 0}
    for f in findings:
        counts[f["level"]] = counts.get(f["level"], 0) + 1
        rel = os.path.relpath(f["path"], root)
        print(f"{f['level'].upper():<8} {rel}: {f['message']}")
    print("-" * 50)
    print(f"{len(files)} skills | {counts['error']} errors, "
          f"{counts['warning']} warnings, {counts['info']} info")
    if counts["error"] or (getattr(args, "strict", False) and counts["warning"]):
        sys.exit(1)
...
def main():
    ...
    init_parser.add_argument("--agents",
                             help="Comma-separated agent keys to enable (e.g. claude,cursor,antigravity)")
    init_parser.add_argument("--all-agents", action="store_true",
                             help="Enable all AI agents (non-interactive)")
    init_parser.add_argument("--no-commands", action="store_true",
                             help="Skip generating /aim-<skill> slash commands")
    init_parser.add_argument("--git-track", action="store_true",
                             help="Track generated files in git (default)")
    init_parser.add_argument("--git-ignore", action="store_true",
                             help="Add generated files to .gitignore")
    ...
    add_mem.add_argument("--importance", type=int, help="Importance 1-10 (default 5); biases recall ranking")

    context_mem = mem_sub.add_parser("context", help="Print the most relevant memories as a context block (for agents)")
    context_mem.add_argument("query", nargs="?", help="What you're working on (optional; omit for top memories overall)")
    context_mem.add_argument("--limit", type=int, default=8, help="Max memories to include (default 8)")
    ...
    new_spec_p = spec_sub.add_parser("new", help="Scaffold a new spec (requirements/design/tasks with EARS criteria)")
    new_spec_p.add_argument("name", help="Feature / spec name")
    new_spec_p.add_argument("--no-task", action="store_true", help="Skip creating the umbrella task")
    ...
    # upgrade
    subparsers.add_parser("upgrade", help="Upgrade AIM to the latest release via pip")

    # lint
    lint_parser = subparsers.add_parser("lint", help="Lint SKILL.md files against the Agent Skills conventions")
    lint_parser.add_argument("path", nargs="?", default=".", help="Directory to scan recursively (default: current dir)")
    lint_parser.add_argument("--strict", action="store_true", help="Exit non-zero on warnings too (CI-friendly)")
    lint_parser.add_argument("--fix", action="store_true", help="Fold non-standard `when_to_use` into `description` in place")
    ...
    # Opportunistic, non-blocking "new release available" notice. Reads a local
    # cache only (no latency); silent unless interactive. Never raises.
    try:
        _update_check().maybe_notify(__version__, command=args.command)
    except Exception:
        pass
