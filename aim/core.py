...
def _semantic():
    """Lazy import of the optional embeddings backend. Always importable;
    its own `available()` reports whether the [semantic] extra is installed."""
    try:
...
# Retrieval weighting by memory type/category: durable knowledge (corrections,
# decisions) should outrank one-off notes. Unknown categories default to 1.0.
CATEGORY_WEIGHT = {
    "correction": 1.3,
    "decision": 1.2,
    "rule": 1.15,
...
def _clamp_importance(value):
    """Coerce an importance score into the 1..10 range (default when unset)."""
    try:
        n = int(value)
...
def add_memory(content, category="general", layer="project", author=None, refs=None,
    ...
    `importance` (1..10) biases retrieval ranking; defaults to 5.
    Raises ValueError if content is empty / whitespace-only."""
    cli = _cli()
...
# ==========================================
# Memory retrieval (ranking + assembled context)
# ==========================================
def _recency_score(iso_value, now=None):
    """Exponential recency in (0, 1]: ~1.0 today, 0.5 after one half-life."""
...
def rank_memories(query=None, limit=10, layer=None, now=None):
    """Rank active memories by relevance × importance × recency × type-weight.

    Relevance uses semantic embeddings when the [semantic] extra is installed,
    otherwise a deterministic substring match. With no query, relevance is
    uniform, so the result is an importance/recency-ranked "what matters most"
    list. Returns a list of memory dicts, highest-ranked first."""
    mems = [m for m in load_memories() if m.get("status", "active") == "active"]
    if layer:
    ...
    if query:
        sem = _semantic()
        ranked = (sem.rank(query, [(m["id"], m.get("content", "")) for m in mems])
                  if sem.available() else None)
        if ranked is not None:
            relevance = {mid: max(0.0, score) for mid, score in ranked}
        else:
            # Deterministic keyword fallback: fraction of query tokens present.
            ...
            for m in mems:
                ...
                if not q_tokens:
                    ...
                    relevance[m["id"]] = 0.2 + 0.8 * (hits / len(q_tokens))
    else:
        relevance = {m["id"]: 1.0 for m in mems}

    scored = []
    for m in mems:
        importance = _clamp_importance(m.get("importance")) / 10.0
        recency = _recency_score(m.get("reviewedAt") or m.get("createdAt"), now)
        cat_w = CATEGORY_WEIGHT.get((m.get("category") or "").lower(), 1.0)
        rel = relevance.get(m["id"], 0.0)
        score = rel * (0.4 + 0.6 * importance) * (0.3 + 0.7 * recency) * cat_w
        scored.append((score, m))
    scored.sort(key=lambda s: -s[0])
    return [m for _s, m in scored[:limit]]


def get_memory_context(query=None, limit=8, max_chars=2000):
    """Assemble the most relevant memories into a compact markdown block an
    agent can inject as working memory. Honours a character budget (a token
    proxy). Returns '' when there is nothing to surface."""
    mems = rank_memories(query=query, limit=limit)
    if not mems:
        return ""
    header = "# Relevant project memory" + (f" — query: {query}" if query else "")
    lines = [header, ""]
    used = len(header) + 1
    shown = 0
    for m in mems:
        ...
        refs = m.get("refs") or []
        if refs:
            entry += f"  (refs: {', '.join(refs[:3])})"
        if used + len(entry) + 1 > max_chars and shown > 0:
...
def task_to_issue_body(task):
    ...
    if task.get("ac"):
        ...
        for ac in task["ac"]:
            ...
            lines.append(f"- [{box}] {ac.get('text', '')}")
        lines.append("")
...
SPEC_REQUIREMENTS_TMPL = """# Requirements: {name}

## Introduction

<One or two paragraphs: the problem, who has it, and the desired outcome.>
...
1. WHEN <trigger event> THE SYSTEM SHALL <observable response>.
2. WHILE <in some state> THE SYSTEM SHALL <observable response>.
3. IF <unwanted condition> THEN THE SYSTEM SHALL <observable response>.
4. WHERE <a feature is included> THE SYSTEM SHALL <observable response>.

<!-- Add R2, R3, ... Keep each criterion atomic and testable. -->
"""

SPEC_DESIGN_TMPL = """# Design: {name}

## Overview

<How the solution works at a high level. Reference requirements R1, R2, ...>

## Architecture

<Components and how they interact. A small diagram or bullet list is fine.>
...
<New or changed data shapes, files, schemas.>
...
SPEC_TASKS_TMPL = """# Tasks: {name}

> Dependency-ordered. `[P]` marks tasks that can run in parallel.
> Check items off as you complete them; keep this in sync with `aim task`.

- [ ] T1: <first task>  (file: <path>)
- [ ] T2 [P]: <parallel task>  (file: <path>)
- [ ] T3: <depends on T1>  (file: <path>)

<!-- Expand these into tracked tasks with the MCP `decompose_prd` prompt,
     or `aim task create ... --depends-on`. -->
"""


def create_spec(name, with_task=True):
    """Scaffold a spec-driven feature folder under
    .ai-context/docs/specs/<slug>/ with requirements.md (EARS criteria),
    design.md, and tasks.md, plus an optional umbrella task linked to the
    requirements doc. Returns a summary dict. Raises ValueError on bad input
    ...
    slug = re.sub(r"[^\w]+", "-", name).strip("-").lower() or "spec"
    spec_dir = os.path.join(cli.DOCS_DIR, "specs", slug)
    if os.path.exists(spec_dir):
        raise ValueError(f"spec '{slug}' already exists at {spec_dir}")
    os.makedirs(spec_dir)

    docs = {}
    for fname, tmpl in (
        ("requirements.md", SPEC_REQUIREMENTS_TMPL),
        ("design.md", SPEC_DESIGN_TMPL),
        ("tasks.md", SPEC_TASKS_TMPL),
    ):
        rel = f"specs/{slug}/{fname}"
        with open(os.path.join(cli.DOCS_DIR, rel), "w", encoding="utf-8") as f:
            f.write(tmpl.format(name=name))
        docs[fname] = rel

    result = {"name": name, "slug": slug, "dir": spec_dir, "docs": docs, "taskId": None}
    if with_task:
        meta = create_task(
            f"Implement: {name}",
            description=f"Umbrella task for spec '{name}'. See requirements/design/tasks.",
            spec=f"@doc/{docs['requirements.md']}",
        )
        result["taskId"] = meta["id"]
    return result
...
def run_diagnostics(author=None):
    ...
    for mem in load_memories():
        ...
        _first_line = (mem.get('content', '')[:48].splitlines() or [''])[0]
        label = f"Memory #{mem.get('id')} \"{_first_line}\""
...
def _read_skill_frontmatter(text):
    """Parse a leading `--- ... ---` block into {key: value} (one level, quotes
    stripped). Returns (fields, has_frontmatter)."""
...
def find_skill_files(root):
    """Return every SKILL.md path under `root` (recursive), sorted."""
    out = []
...
def lint_skills(root):
    """Validate every SKILL.md under `root` against the Agent Skills
    conventions. Returns findings: [{path, level: error|warning|info, message}].
    Deterministic; no network, no LLM."""
    findings = []

    def add(path, level, message):
        findings.append({"path": path, "level": level, "message": message})
    ...
    for path in find_skill_files(root):
        try:
            ...
            add(path, "error", f"cannot read: {e}")
            continue

        folder = os.path.basename(os.path.dirname(path))
        fields, has_fm = _read_skill_frontmatter(text)
        if not has_fm:
            add(path, "error", "missing YAML frontmatter (--- ... ---)")
            continue
        ...
        if not name:
            add(path, "error", "frontmatter missing required `name`")
        else:
            if not _SKILL_NAME_RE.match(name):
                add(path, "error", f"`name` '{name}' must be lowercase-hyphen (a-z, 0-9, -)")
            if name != folder:
                add(path, "warning", f"`name` '{name}' does not match folder '{folder}'")
            if "claude" in name.lower() or "anthropic" in name.lower():
                add(path, "warning", "`name` should not contain 'claude'/'anthropic'")

        if not desc:
            add(path, "error", "frontmatter missing required `description`")
        else:
            if len(desc) > SKILL_DESC_MAX:
                add(path, "warning", f"`description` is {len(desc)} chars (> {SKILL_DESC_MAX})")
            low = desc.lower()
            if not any(w in low for w in ("use ", "when", "activate", "for ")):
                add(path, "info", "`description` should also state WHEN to use the skill")

        if "when_to_use" in fields:
            add(path, "info", "non-standard `when_to_use` field; fold it into `description` for portability")

        n_lines = text.count("\n") + 1
        if n_lines > SKILL_MAX_LINES:
            add(path, "warning",
                f"SKILL.md is {n_lines} lines (> {SKILL_MAX_LINES}); split detail into reference files (progressive disclosure)")

    return findings


def _yaml_safe_unquoted(value):
    """Sanitize a string so it round-trips as an UNQUOTED YAML scalar through
    AIM's naive quote-stripping parsers (no `: `, `#`, quotes, or leading
    indicators)."""
    value = value.replace('"', "'").replace("\t", " ")
    value = re.sub(r":\s", " - ", value)        # avoid key:value confusion
    value = re.sub(r"\s+", " ", value).strip()
    value = value.lstrip("#>|*&!%@`[]{}-").strip()
    return value


def normalize_skills(root):
    """Fold each SKILL.md's non-standard `when_to_use` into its `description`
    (the portable Agent Skills convention) under `root`. Safe: it re-parses the
    rewritten file and skips any change that would break the frontmatter.
    Returns [{path, status: fixed|unchanged|skipped, detail}]."""
    results = []
    for path in find_skill_files(root):
        try:
            with open(path, "r", encoding="utf-8") as f:
            ...
            results.append({"path": path, "status": "skipped", "detail": str(e)})
            continue

        bom = "\ufeff" if raw.startswith("\ufeff") else ""
        text = raw[len(bom):]
        if not text.startswith("---"):
            results.append({"path": path, "status": "skipped", "detail": "no frontmatter"})
            continue

        lines = text.splitlines()
        end = next((i for i in range(1, len(lines)) if lines[i].strip() == "---"), None)
        if end is None:
            results.append({"path": path, "status": "skipped", "detail": "unterminated frontmatter"})
            continue

        fm = lines[1:end]
        desc_idx = wtu_idx = None
        for i, ln in enumerate(fm):
            key = ln.split(":", 1)[0].strip() if ":" in ln else ""
            if key == "description" and desc_idx is None:
                desc_idx = i
            elif key == "when_to_use" and wtu_idx is None:
                wtu_idx = i

        if wtu_idx is None:
            results.append({"path": path, "status": "unchanged", "detail": "no when_to_use"})
            continue
        if desc_idx is None:
            results.append({"path": path, "status": "skipped", "detail": "when_to_use without description"})
            continue

        def _val(ln):
            return ln.split(":", 1)[1].strip().strip('"').strip("'").strip()

        desc, wtu = _val(fm[desc_idx]), _val(fm[wtu_idx])
        folded = desc
        if wtu[:25].lower() not in desc.lower():       # don't duplicate
            if folded and folded[-1] not in ".!?":
                folded += "."
            folded = f"{folded} {wtu}".strip()
        folded = _yaml_safe_unquoted(folded)

        new_fm = [f"description: {folded}" if i == desc_idx else ln
                  for i, ln in enumerate(fm) if i != wtu_idx]
        body = "\n".join(lines[end + 1:])
        new_text = bom + "---\n" + "\n".join(new_fm) + "\n---\n" + body
        if not new_text.endswith("\n"):
            new_text += "\n"

        fields, ok = _read_skill_frontmatter(new_text)
        if not ok or not fields.get("description") or "when_to_use" in fields:
            results.append({"path": path, "status": "skipped", "detail": "fix would break frontmatter"})
            continue
        ...
        try:
            ...
            results.append({"path": path, "status": "skipped", "detail": str(e)})
            continue
        results.append({"path": path, "status": "fixed", "detail": "folded when_to_use into description"})
