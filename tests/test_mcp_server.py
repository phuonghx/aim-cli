...
def test_tools_list_shapes():
    ...
    assert names == {"list_tasks", "get_task", "create_task", "create_tasks",
                     "next_task", "search", "get_memory_context", "add_memory",
                     "list_memories", "record_correction", "review_memory", "doctor"}
    for t in tools:
...
def test_tool_get_memory_context(workspace):
    is_err, payload = call_tool("add_memory", {
        "content": "use the fetch wrapper in src/lib/http.ts",
        "category": "decision", "importance": 9})
    assert not is_err
    assert json.loads(payload)["importance"] == 9

    is_err, payload = call_tool("get_memory_context", {"query": "http"})
    assert not is_err
    ctx = json.loads(payload)["context"]
    assert "fetch wrapper" in ctx and "[decision]" in ctx
